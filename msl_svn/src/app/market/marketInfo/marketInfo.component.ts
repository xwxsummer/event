...
import { Scan } from '../../fragment/scan';//扫一扫
declare var mui: any;
declare var $: any;
...
//截取金额
    ...
    //img1="<img src='../assets/images/select.png'>";
    //img2="<img src='../assets/images/select2.png'>";
    ngOnInit() {
        ...
        map.plugin(['AMap.Geolocation', 'AMap.DistrictSearch'], function() {
          console.log('定位插件、行政区插件');
            ...
            if(sessionStorage['gpsLng']){//如果sessionStorage有当前位置,不定位
            ...
            .then(data => {
                ...
                if(data.code==0){
                    ...
                    for(let s=0;s<data.data.length;s++){
                       if(data.data[s].headImg=="null"){//
                           data.data[s].headImg="../assets/images/headImg.png"
                       }else{
                           data.data[s].headImg=data.data[s].domain+data.data[s].headImg;
                       }
                    }
                }
