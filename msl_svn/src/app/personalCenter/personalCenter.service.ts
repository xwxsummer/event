...
//上传图片
  ...
  //头像图片上传图片
  public upBase64ImageHead(base64FileList:string,fixFileName:string):Promise<any>{
    let type:string = 'user';
    let headers = new Headers();
    headers.append('Content-Type','application/x-www-form-urlencoded');
    return this.http.post(Api.base64Image,
        `base64FileList=${encodeURIComponent(base64FileList)}&type=${type}&fixFileName=${fixFileName}`,
        {headers : headers}
    ).toPromise();
  }
  //查看认证信息接口
...
//根据主单编号取消主单剩余货源this.localStorage.getObject("data").userId
  ...
  //移除银行卡
  public unBinding(walletCode,id,psd):Observable<any>{
    //saveDTO.bankCode="CMB";
    let params = new URLSearchParams();
    params.set("userCode",walletCode);
    params.set("id",id);
    psd=Md5.hashStr(psd)+"";
    params.set("pwd",psd);
  ...
  //重置秘密第一步
  public walletCheckMobile(walletCode,captcha):Observable<any>{
    let params = new URLSearchParams();
    params.set("userCode",walletCode);
    params.set("captcha",captcha);
    return this.ajax.myPost(Api.walletCheckMobile,params);
  }
