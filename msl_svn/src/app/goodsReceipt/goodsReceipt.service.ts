...
export class goodsReceiptService{
    ...
    public getOrderChildListByReceiveId(page):Observable<any>{
        //http://192.168.1.230:8100/order-service/order/child/getOrderChildListBySendId?page=1&sendId=93&pageSize=5+this.local_Storage.userId
         return this.ajax.myget(Api.getOrderChildListByReceiveId+"?page="+page+"&receiveId="+this.local_Storage.userId+"&pageSize=5")
            .map((res: Response) => res);
    }
   public getOrderDetailsByChildNo(OrderNo):Observable<any>{
        return this.ajax.myget(Api.getOrderDetailsByChildNo+"?childNo="+OrderNo+"&userCode="+this.local_Storage.walletCode)
            .map((res: Response) => res);
    }
