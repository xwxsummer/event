...
export class EvaluationDetails implements OnInit{
  ...
  ngOnInit(): void{
    ...
    this.service.getDriverEvaluateById(driverId,this.activatedRoute.snapshot.params['id'])
      .subscribe(data => {
        if(data.code==0){
          this.data = data.data;
