...
export class ValidateMessage{
    ...
    public validateMobile(mobile):number{
        if(mobile){//手机号码不为空
            if(mobile.length<11){
                mui.toast("手机号码不足<br/>11位，请填写",{ duration:'short', type:'div' });
                return 2;
    ...
    public validateCaptchaReg(captcha):number{
        if(captcha){//验证码 不为空
            if(captcha.length<4){
                mui.toast("验证码不足<br/>4位，请填写",{ duration:'short', type:'div' });
                return 2;
    ...
    public validatePsd(psd):number{
        if(psd){//密码不为空
            if(psd.length<6){
                mui.toast("密码不足<br/>6位，请填写",{ duration:'short', type:'div' });
                return 2;
    ...
    public validateCardNo(cardNo):number{
        if(cardNo){//身份证号码不为空
            if(cardNo.length<18){
                mui.toast("请填写身份证号码",{ duration:'short', type:'div' });
                return 2;
