...
//验证手机号
  ...
  * */
  ...
  public countPublishOrder(beginTime,endTime):Observable<any>{
    return this.ajax.myget(Api.countPublishOrder+"?userId="+this.localStorage.getObject("data").userId+"&beginTime="+beginTime+"&endTime="+endTime);
  }
  //已经接单的数目列表
  public countOrderChildsToList(beginTime,endTime,status):Observable<any>{
    return this.ajax.myget(Api.countOrderChildsToList+"?userId="+this.localStorage.getObject("data").userId+"&beginTime="+beginTime+"&endTime="+endTime+"&status="+status);
  }
  // 已经运送的货物数量列表，已支付运费的列表
  public countCompleteOrderToList(beginTime,endTime):Observable<any>{
     return this.ajax.myget(Api.countCompleteOrderToList+"?userId="+this.localStorage.getObject("data").userId+"&beginTime="+beginTime+"&endTime="+endTime);
...
//根据主单编号取消主单剩余货源this.localStorage.getObject("data").userId
  ...
  public getOrderChildListByParam(orderInfoTermDTO){
    orderInfoTermDTO.ownerId=this.localStorage.getObject("data").userId
